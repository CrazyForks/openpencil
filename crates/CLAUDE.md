...
| i18n              | 15 canonical locale catalogs (765 main + 325 Git = 1,090 direct keys each) in the `op-i18n` crate                                  | `op-i18n/src/i18n/{en,zh_cn,zh_tw,ja,ko,fr,es,de,pt,ru,hi,tr,th,vi,id}.rs`                                                              |
...
- Locale tables are hand-maintained in Rust. Add or change the corresponding direct entry in all relevant main/Git locale modules, then run:

  ```sh
  cargo test -p op-i18n
  ```

  Catalog integrity tests enforce the exact cross-locale key set, reject duplicate or unsupported match arms, and require every translation to preserve the English placeholder set. `tools/convert-locales.py` is a deliberate failing shim because its retired TypeScript source no longer exists. Keep each locale file under the repository's 800-line ceiling. Runtime lookup still falls back through English and then the raw key for debug visibility, while the tests prevent shipped locale gaps.
