...
pub async fn discover_builtin_models(
    ...
    tokio::time::timeout(REQUEST_TIMEOUT, discover_with_deadline(config, access))
        .await
...
async fn discover_with_deadline(
    ...
    let client = crate::provider_dial::client_for(dial_policy, request.url.as_str())
        .await
        .map_err(|_| BuiltinModelDiscoveryError::RequestFailed)?;
    let response = client
        .get(request.url)
...
fn discovery_request(
    config: &BuiltinAgentConfig,
) -> Result<DiscoveryRequest, BuiltinModelDiscoveryError> {
...
fn parse_catalog(
    ...
    for item in items {
        ...
        if explicit_non_chat_model(id) || !seen.insert(id.to_string()) {
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn gemini_parser_keeps_only_generate_content_models() {
        let body = br#"{"models":[
