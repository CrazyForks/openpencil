...
//!
//! Two seams still speak `String` on purpose, and both are documented where
//! they are defined: [`super::write_mcp_http_response`] (the permissive-CORS
//! wrapper) and `doc_sync`'s document-sync validation. `mcp_live.rs` returns
//! the former's `Result` DIRECTLY from its own `Result<_, String>` handlers
//! and formats the latter's error as a `&str`, so converting them would
//! ripple into a module this pass does not own. Everything `mcp_live`
//! reaches through `?` converts via [`From<McpServeError> for String`]
//! instead, which is why that bridge exists.
...
/// `mcp_live.rs`'s per-connection handlers and `cli_modes.rs`. `Display` is
/// transparent, so a `?` through this conversion produces the same message
/// the pre-conversion code returned. Delete it once those modules carry
/// typed errors of their own.
impl From<McpServeError> for String {
    fn from(error: McpServeError) -> String {
        error.to_string()
    }
}
