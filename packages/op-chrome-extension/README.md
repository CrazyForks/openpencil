...
Manifest V3, no dependencies, no external network access. The extension's
logic is **Rust compiled to WebAssembly**; the JavaScript is a thin layer over
the `chrome.*` APIs (see [Architecture](#architecture)).
...
Requires Chrome 103+ (the first release that accepts `'wasm-unsafe-eval'` in
an extension page's CSP).
...
| `background.js`                | Service worker. Owns the element-pick flow, which outlives the popup (see [Capture element](#capture-element)).                                                                                               |
| `client.js`                    | `fetch`, `AbortController`, reading the response body — plus the one `JSON.stringify` that embeds the snapshot in the `/mcp` envelope, for the reason below. Also the account upload.                         |
...
- a **badge** on the toolbar icon — green `✓` or red `✗`, cleared after 90
  seconds or as soon as you open the popup;
...
badge needs no locale. A pick that finishes while the popup is closed is stored
as message _keys_ plus their substitutions rather than as rendered text, so
switching language between the pick and the next popup open still reads right.

`scripts/check-locales.mjs` (part of `bun run lint`, alongside the extractor and
service-worker guards) fails on any drift: a key present in one catalog and not
another, a message whose `$n` slots do not match English's, an empty string,
invalid JSON, a `_locales/` directory with no row in the switcher or the
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `POST /api/import/web-snapshot`                | desktop app's live MCP endpoint (`crates/op-host-services/src/mcp_live/snapshot_ingest.rs`) | Insert-only, no token needed. The one route on that endpoint reachable from a `chrome-extension://` origin.      |
...
The endpoint box accepts loopback only — `127.0.0.1:<port>`, `[::1]:<port>`, or
`localhost:<port>`, which is rewritten to `127.0.0.1` (the live endpoint refuses
...
The ingest route accepts a `chrome-extension://<id>` origin, and by default any
well-formed one: this extension is unpublished, so an unpacked load derives a
different id on every machine and there is no stable id to pin. An accepted
origin buys exactly one insert-only tool, and the reply's
...
`[::1]` is a different story. Chrome's match-pattern grammar has no syntax for
an IPv6 host — `http://[::1]/*` is rejected outright and would make the whole
manifest fail to load — so an `[::1]:<port>` endpoint cannot be covered by
`host_permissions`. The endpoint box still accepts it, and it still works when
the editor answers the extension's origin with CORS headers, but it is the one
endpoint form that depends on the server's CORS reply rather than on a granted
host permission. **Prefer `127.0.0.1:<port>`.**
...
background.js                    Service worker: the element-pick flow, badge, stored result
capture.js                       Injection harness + chunked transfer
picker.js                        Hover/click element picker injected into the page
client.js                        The two local ingresses + the account upload (fetch only)
...
`node --check`s every runtime script, runs the three drift guards (extractor,
service-worker imports, 15 locales), stages **only** the runtime files, and
writes `dist/op-chrome-extension-<version>.zip` with the manifest at the
archive root. Scripts, docs, `.gitignore` and the crate's `pkg/` never enter
it. The listing at the end of the run is what the store will receive.
...
| Data usage disclosures    | No data collected for analytics, advertising, or sale. Website content is handled only on user action and sent only to the user's own destination. |
...
2. Set it on every OpenPencil that should accept captures from it:

   ```bash
   OPENPENCIL_EXTENSION_ALLOWED_IDS=<id> openpencil-desktop
   ```

   The editor's ingest route admits any `chrome-extension://` origin when this
   is unset; setting it narrows admission to the ids you name, which is what
   you want once a published id exists — a side-loaded copy of this extension
   can no longer post into your editor.
...
**"Endpoint must be loopback with a port"** — only `127.0.0.1`, `[::1]`, and
`localhost` are accepted. The extension will not post a page capture anywhere
else.
